...
def test_list_todos_should_return_5_todos(session, client, token, user):
    ...
    # bul_save_objects faz o add de todas as 5
    # todos geradas por TodoFactory
...
def test_list_todos_filter_combined__should_return_5_todos(
...
def test_patch_todo(session, client, user, token):
    ...
    response = client.patch(
        f'/todos/{todo.id}',
        headers={'Authorization': f'Bearer {token}'},
        json={'title': 'title_test'},
    )

    assert response.status_code == HTTPStatus.OK
    assert response.json()['title'] == 'title_test'
